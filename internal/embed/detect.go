...
func DetectEmbeddedZip() (*ZipInfo, error) {
	exePath, err := os.Executable()
	if err != nil {
		return nil, fmt.Errorf("getting executable path: %w", err)
	}
	return DetectEmbeddedZipFromPath(exePath)
}

// DetectEmbeddedZipFromPath checks if the file at the given path has a zip archive appended.
func DetectEmbeddedZipFromPath(path string) (_ *ZipInfo, err error) {
	f, err := os.Open(path)
	if err != nil {
