...
func TestParseInputSchemaInvalidJSON(t *testing.T) {
	schema := parseInputSchema(json.RawMessage(`not json`))
...
func TestPrintServerHelp(t *testing.T) {
	...
	printServerHelp("test-server", tools)
}
...
func TestPrintToolHelp(t *testing.T) {
	...
	// Just verify it doesn't panic
	printToolHelp("test-server", tool)
}
...
func TestParseToolArgsPassthrough(t *testing.T) {
	emptySchema := ParsedSchema{Properties: map[string]PropertyInfo{}}
