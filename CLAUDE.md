...
- `cli.ParsedSchema` / `cli.PropertyInfo` — parsed JSON schema for flag generation
...
**Compile**: config → parallel `mcp.Connect`+`ListTools` per server → `compile.CreateZipArchive` → append zip to binary copy
